...
from .old_net import Net
from .old_utils_network import *


# todo put all basic functions such as concat,conv,pool...  in an other file
...
class InceptionV1(Net):
    ...
    # todo to use later, not necessary in the simplified version
    ...
    def _flatten_and_softmax(self, layer_name, inputs, out_nodes):
        ...
        with tf.variable_scope(layer_name) as scope:
            ...
            flat_x_biased = tf.nn.bias_add(tf.matmul(flat_x, w), b)
            output = tf.nn.softmax(flat_x_biased) # todo I replaced relu with softmax here
            return output
    ...
    def _inception1(self, inputs):  # todo find the right sizes in david's code
        # todo : here we consider the naive version with no reduction
        conv_1x1 = conv2d('inception1_conv_1x1', inputs=inputs, out_channels=6, kernel_size=2, strides=1)
        conv_3x3 = conv2d('inception1_conv_3x3', inputs=inputs, out_channels=10, kernel_size=2, strides=1)
        conv_5x5 = conv2d('inception1_conv_5x5', inputs=inputs, out_channels=5, kernel_size=2, strides=1)
        maxpool_3x3 = max_pool('inception1_pool_3x3', inputs=inputs, pool_size=3, strides=1)
        output = concat('inception1_output', [conv_1x1, conv_3x3, conv_5x5, maxpool_3x3]) #25 channels
        return output

    def _inception2(self, inputs):  # todo find the right sizes in david's code
        # todo : here we consider the naive version with no reduction
        conv_1x1 = conv2d('inception2_conv_1x1', inputs=inputs, out_channels=5, kernel_size=2, strides=1)
        conv_3x3 = conv2d('inception2_conv_3x3', inputs=inputs, out_channels=7, kernel_size=2, strides=1)
        conv_5x5 = conv2d('inception2_conv_5x5', inputs=inputs, out_channels=4, kernel_size=2, strides=1)
        maxpool_3x3 = max_pool('inception2_pool_3x3', inputs=inputs, pool_size=3, strides=1)
        output = concat('inception2_output', [conv_1x1, conv_3x3, conv_5x5, maxpool_3x3]) #19 channels
        return output

    def _inception3(self, inputs):  # todo find the right sizes in david's code
        # todo : here we consider the naive version with no reduction
        conv_1x1 = conv2d('inception3_conv_1x1', inputs=inputs, out_channels=5, kernel_size=2, strides=1)
        conv_3x3 = conv2d('inception3_conv_3x3', inputs=inputs, out_channels=4, kernel_size=2, strides=1)
        conv_5x5 = conv2d('inception3_conv_5x5', inputs=inputs, out_channels=3, kernel_size=2, strides=1)
        maxpool_3x3 = max_pool('inception3_pool_3x3', inputs=inputs, pool_size=2, strides=1)
        output = concat('inception3_output', [conv_1x1, conv_3x3, conv_5x5, maxpool_3x3]) #14 channels
        return output

    def build_model(self):
        conv1_2x2 = conv2d('conv1_2x2', inputs=self.x, out_channels=16, kernel_size=2, strides=1)
        pool1_2x2 = max_pool('pool1_2x2', inputs=conv1_2x2, pool_size=2,
                                  strides=1)  # todo pooling here does not reduce dimension need to be modified ?
        conv1_2x2 = conv2d('conv2_2x2', inputs=pool1_2x2, out_channels=8, kernel_size=2, strides=1)
        pool2_2x2 = max_pool('pool2_2x2', inputs=conv1_2x2, pool_size=2, strides=1)

        inception1 = self._inception1(inputs=pool2_2x2)
        pool3_2x2 = max_pool('pool3_2x2', inputs=inception1, pool_size=2, strides=1)

        inception2 = self._inception2(inputs=pool3_2x2)
        pool4_2x2 = max_pool('pool4_2x2', inputs=inception2, pool_size=2, strides=1)

        inception3 = self._inception3(inputs=pool4_2x2)
        pool5_2x2 = avg_pool('pool5_2x2', inputs=inception3, pool_size=2, strides=1)

        dropout_tensor = dropout('dropout_10',inputs=pool5_2x2,keep_prob=0.9)
        self.logits = self._flatten_and_softmax('output_softmax', dropout_tensor, out_nodes=self.config.n_classes)


        ############### CODE FROM GIT WITH THE COMPLET GOOGLENET
